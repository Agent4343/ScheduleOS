...
export function CoverageCard({ isAdmin }: { isAdmin: boolean }) {
  ...
  const submitRole = async (e: FormEvent) => {
    ...
    if (!roleForm) return
    try {
      await saveRole.mutateAsync({ ...roleForm, requiredQualification: roleForm.requiredQualification.trim() || null })
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              {isAdmin && !roleForm && (
                <Button size="sm" variant="ghost" onClick={() => setRoleForm({ ...EMPTY_ROLE, sortOrder: roleList.length + 1 })}>
            ...
            {roleForm && (
              <form onSubmit={submitRole} className="p-3 border rounded-lg bg-muted/50 space-y-3">
                ...
                <div className="flex gap-2">
                  <Button size="sm" type="submit" disabled={saveRole.isPending}>{saveRole.isPending ? "Saving…" : "Save"}</Button>
                  <Button size="sm" type="button" variant="outline" onClick={() => setRoleForm(null)}>Cancel</Button>
                </div>
              </form>
            )}
                ...
                <li key={r.id} className="flex items-center justify-between gap-2 p-2 border rounded text-sm">
                  ...
                  {isAdmin && (
                    <div className="flex shrink-0">
                      <Button variant="ghost" size="sm" aria-label={`Edit ${r.name}`} onClick={() => setRoleForm({ id: r.id, name: r.name, sortOrder: r.sortOrder, minDay: r.minDay, targetDay: r.targetDay, minNight: r.minNight, targetNight: r.targetNight, requiredQualification: r.requiredQualification ?? "" })}>
