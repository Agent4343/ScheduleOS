...
**IMPORTANT:** Do not commit secrets into build configurations or the repository. Set `NEXTAUTH_SECRET` and `RESEND_API_KEY` as environment variables in Railway (Project > Variables) or your hosting provider. If you accidentally committed secrets, rotate them immediately and remove them from history.
