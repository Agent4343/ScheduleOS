...
async function main() {
  ...
  // Optional: Create demo users (opt-in only)
  // For local development, create an admin manually using Prisma Studio (npx prisma studio)
  // or use the SEED_DEMO environment variable approach below.
  if (process.env.SEED_DEMO === "true") {
    if (!process.env.SEED_DEMO_PASSWORD) {
      ...
      console.warn(
        "   To create demo users, set SEED_DEMO_PASSWORD: SEED_DEMO=true SEED_DEMO_PASSWORD=yourpassword npm run db:seed"
      )
    } else {
      ...
      const workers = [
        {
          name: "Worker A1",
          email: "worker.a1@local",
          crew: "Crew A",
          position: "Operator",
          positionType: "OPERATOR" as const,
        },
        {
          name: "Worker A2",
          email: "worker.a2@local",
          crew: "Crew A",
          position: "Control Room",
          positionType: "ONSHORE_CONTROL_ROOM" as const,
        },
      ]
